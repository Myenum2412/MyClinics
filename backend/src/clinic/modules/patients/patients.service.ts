...
export class PatientService {
  ...
  async createPatient(ctx: ClinicContext, input: CreatePatientInput): Promise<WithId<PatientDoc>> {
    ...
    if (!input.skipNotification) {
      const isAutoGenerated = ctx.userId === "system" || Boolean(input.email && input.email.endsWith("@patient.mc"));
  ...
  /**
   * Triggers the welcome notification to the patient, fetching any uploaded registration
   * files and formatting them in the welcome WhatsApp message.
   */
  async sendWelcomeMessage(
    ctx: ClinicContext,
    patientId: string,
    opts: { sendCredentials?: boolean; password?: string | null }
  ): Promise<void> {
    const clinicId = requireClinicOf(ctx);
    const patient = await this.repo(ctx).findByPatientId(patientId);
    if (!patient) throw new NotFoundError("Patient not found");
    ...
    const files = await this.db
      .collection(CLINIC_COLLECTIONS.medicalRecordFiles)
      .find({ clinicId, patientId, deletedAt: { $exists: false } })
      .toArray();
    ...
    const patientDocuments = await Promise.all(
      files.map(async (f) => {
        const downloadUrl = await getDownloadUrl(f.r2Key);
        return {
          fileName: f.fileName,
          size: f.size,
          downloadUrl,
    ...
    const isAutoGenerated = ctx.userId === "system" || Boolean(patient.email && patient.email.endsWith("@patient.mc"));

    await notifyPatientRegistered(this.db, patient, {
      sendCredentials:
        !isAutoGenerated &&
        !!opts.sendCredentials &&
        Boolean(patient.email && opts.password),
      portalUsername: patient.email ?? null,

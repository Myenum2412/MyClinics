...
export default async function SettingsPage() {
  const [soulRes, docsRes] = await Promise.all([
    apiFetch<SoulResponse>("/api/soul"),
    apiFetch<KnowledgeResponse>("/api/knowledge"),
  ]);

  const soul =
    soulRes.status === 200 && soulRes.data.soul
      ? soulRes.data.soul
      : { content: "", fallbackReply: "", version: 0 };
  const documents =
    docsRes.status === 200 ? docsRes.data.documents : [];

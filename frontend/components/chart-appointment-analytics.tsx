...
export interface ChartAppointmentAnalyticsDatum {
  date: string;
  total: number;
  completed: number;
}
...
export function ChartAppointmentAnalytics({
  title,
  subtitle,
  data,
}: {
  title: string;
  subtitle: string;
  data: ChartAppointmentAnalyticsDatum[];
}) {
  ...
  return (
    <Card className="overflow-hidden rounded-none border-border bg-card shadow-sm py-0">
      <CardHeader className="flex flex-col items-stretch border-b p-0! sm:flex-row">
        ...
        <div className="flex">
          {(["total", "completed"] as const).map((key) => {
            const chart = key;
